    ...
    foreach ($history as $str) {
        if (!(strpos($str, 'Этап согласования №') !== false)){
            ...
            if (is_numeric(strtotime($el[0]))){
                ...
                $arr["sogl#$k"]=$el[2].' '.mb_strcut($el[3],1,2).'.'.mb_strcut($el[4],1,2).'. '.$rsUsers[WORK_POSITION];
                $arr["date_sogl#$k"]="$el[0]";
                $arr["result_sogl#$k"]="$el[6]";
    
            }elseif($el[0]=="Комментарий:"){
